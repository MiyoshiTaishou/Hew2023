...
std::vector<DirectX::SimpleMath::Vector3> ModelRenderer::LoadVertex(const char* FileName)
{
	if (m_ModelPool.count(FileName) > 0)
	{
		m_Model = m_ModelPool[FileName];
		std::vector<Vector3> vec = LoadModelVertex(FileName, m_Model);
		return vec;
	}
...
std::vector<VERTEX_3D> ModelRenderer::GetVertex(const char* FileName)
{
	char dir[MAX_PATH];
	strcpy(dir, FileName);
	...
	while (true)
	{
		...
		else if (strcmp(str, "f") == 0)
		{
			...
			do
			{
				...
				ver.push_back(ModelObj.VertexArray[vc]);
				ic++;
				vc++;
...
int ModelRenderer::GetIndexNum()
{
	return m_Model->SubsetArray->IndexNum;
}
...
std::vector<DirectX::SimpleMath::Vector3> ModelRenderer::LoadModelVertex(const char* FileName, MODEL* Model)
{

	MODEL_OBJ modelObj;
	std::vector<Vector3> vec = LoadObjVertex(FileName, &modelObj);
...
std::vector<Vector3> ModelRenderer::LoadObjVertex(const char* FileName, MODEL_OBJ* ModelObj)
{
	char dir[MAX_PATH];
	strcpy(dir, FileName);
	PathRemoveFileSpec(dir);
