...
// 全頂点をaiMeshから取得する
unsigned int  SkeletalMesh::GetVertices(const aiMesh* aimesh) {
...
// aiMeshから全インデックス取得
unsigned int  SkeletalMesh::GetIndices(const aiMesh* aimesh) {
...
// ベース情報を再計算
void SkeletalMesh::CalcMeshBaseIndex() {
...
// すべてのテクスチャSRVを取得
void SkeletalMesh::GetTextures(const aiScene* aiscene) {
...
// 内蔵テクスチャかどうかを識別する
bool SkeletalMesh::isEmbeddedTexure(const aiTexture* aitexture) {
...
// 個々のマテリアル情報（反射情報）取得
void SkeletalMesh::GetaMatreial(const aiMaterial* aimtrl) {
...
// マテリアル取得
void SkeletalMesh::GetMaterials(const aiScene* aiscene) {
...
ID3D11ShaderResourceView* SkeletalMesh::LoadDiffuseTexture(std::string filename) {
	// ファイルの拡張子を取得
