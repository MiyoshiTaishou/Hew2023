...
Vector3 GetClosestVeretex(const std::vector<Vector3>& vertices, const Vector3 point)
{
	Vector3 closestVertex = vertices[0];
	float closestDistance = CalculateDistance(vertices[0], point);

	for (size_t i = 1; i < vertices.size(); ++i)
	{
		float distance = CalculateDistance(vertices[i], point);
